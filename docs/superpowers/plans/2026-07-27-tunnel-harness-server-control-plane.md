...
`ServerPaths::system()` returns `/etc/rqbit-tunnel`, `/var/lib/rqbit-tunnel`, `/run/rqbit-tunnel`; `ServerPaths::under(root)` exists only for tests/install staging. Add `arc-swap` as a normal workspace dependency and `tempfile` as a dev-dependency to `rqbit-tunnel` in this task. Add `librqbit = { workspace = true, default-features = false, features = ["tracing-subscriber-utils"] }` plus crate features `default = ["default-tls"]`, `default-tls = ["librqbit/default-tls"]`, and `rust-tls = ["librqbit/rust-tls"]`. Cargo features are additive: normal builds use default TLS; Rustls builds MUST use `--no-default-features --features rust-tls` (and pair it with `rqbit/rust-tls` in a workspace build). Requesting both mappings is intentionally rejected by `sha1w`'s existing exactly-one-backend assertion. Do not add a crate-local build script or `compile_error!` guard: Cargo compiles direct dependencies before this crate, so such a guard cannot preempt the dependency error. `ServerStore::open` must set `journal_mode = WAL`, `foreign_keys = ON`, and execute an idempotent migration creating `users`, `traffic_totals`, and `settings` exactly as specified.
