...
`*.rqbt` file. The menu names and imports that file automatically, then installs
the client service definition, enables its autostart, and starts it. With no
bundle or multiple bundles, it prints the discovered state and requires an
explicit path; it never guesses.
